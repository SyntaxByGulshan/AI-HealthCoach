...
export default function MotionStepTracker() {
    ...
    const threshold = 1.2; // Lowered to 1.2 for walking
    const minStepIntervalMs = 250; // Slightly faster to catch fast walking
    ...
    useEffect(() => {
        ...
        const handler = (e: DeviceMotionEvent) => {
            ...
            if (lastAcc.current.length > 4) lastAcc.current.shift(); // Reduced buffer to 4 for better sensitivity
    ...
    return (
        <div className="p-4">
            ...
            <div className="mt-4 text-blue-200 rounded text-xs font-mono">
                <p>Status: {permission}</p>
                <p>Sensor: {typeof DeviceMotionEvent !== 'undefined' ? 'Available' : 'Missing'}</p>
                <p>ReqPerm: {typeof (window as any).DeviceMotionEvent?.requestPermission === 'function' ? 'Yes' : 'No'}</p>
                <p>Acc: {lastAcc.current[lastAcc.current.length - 1]?.toFixed(2) ?? 0}</p>
                <p>Gravity: {gravityRef.current.toFixed(2)}</p>
